...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_rustdesk() {
            if s.contains("Remote Peltic Support")
                && !name.starts_with("upgrade_rustdesk_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("Remote Peltic Support") {
                    s = s.replace("Remote Peltic Support", &app_name);
                } else {
                    // https://github.com/rustdesk/rustdesk-server-pro/issues/845
                    // If app_name contains "Remote Peltic Support" (e.g., "RustDesk-Admin"), we need to avoid
                    // replacing "Remote Peltic Support" within the already-substituted app_name, which would
